      ...
      <div style={{
        ...
        <div style={{ marginBottom: "20px" }}>
          <div style={{ display: "flex", alignItems: "center", gap: "12px", marginBottom: "8px" }}>
            ...
            <span style={{
              background: isLong ? "rgba(0,229,160,0.15)" : "rgba(255,77,109,0.15)",
              border: `1px solid ${isLong ? "rgba(0,229,160,0.4)" : "rgba(255,77,109,0.4)"}`,
              color: isLong ? "#00e5a0" : "#ff4d6d",
              borderRadius: "6px", padding: "3px 10px",
              fontSize: "12px", fontWeight: 700,
            }}>
              {alert.direction} · {alert.timeframe}
            </span>
          </div>
          <div style={{ color: "#666", fontSize: "12px" }}>AI-Powered CRT Analysis</div>
        </div>
...
function generateAlerts(timeframes) {
  const alerts = [];
  let id = 1;
  const patterns = ["SWEEP HIGH", "SWEEP LOW", "RANGE EQ", "BREAKER", "FVG FILL", "OB TEST"];
  const exchanges = ["BINANCE", "BYBIT", "MEXC", "OKX"];

  PAIRS.forEach((pair, pi) => {
    timeframes.forEach((tf, ti) => {
      const seed = pi * 7 + ti * 13 + pair.charCodeAt(0);
      if (seed % 3 === 0) {
        alerts.push({
          id,
          pair,
          exchange: exchanges[seed % exchanges.length],
          direction: seed % 2 === 0 ? "LONG" : "SHORT",
          timeframe: tf,
          pattern: patterns[seed % patterns.length],
          candles: generateMockCandles(pair, tf),
          time: new Date(Date.now() - (seed % 60) * 60000).toLocaleTimeString(),
        });
        id++;
      }
    });
...
export default function CRTTracker() {
  const [activeTimeframes, setActiveTimeframes] = useState(["1D", "4H"]);
    ...
    const interval = setInterval(() => {
      setProgress(p => {
        if (p >= 100) {
          ...
          const newAlerts = generateAlerts(activeTimeframes);
          setAlerts(newAlerts);
